...
const splittedURL = fullURL => {
  ...
  return { main, ref, type: capitalizePlatform(parsedURL.host.split('.')[0]) }
}
...
const gitPathParts = main => {
  ...
  return { main, ref, type: capitalizePlatform('github') }
}
...
const downloadRepo = async repoPath => {
  ...
  try {
    await download(url, tmpDir.path, { extract: true })
  } catch (err) {
    tmpDir.cleanup()
    return false
  }
...
module.exports = { gitPathParts, isRepoPath, fromGit }
